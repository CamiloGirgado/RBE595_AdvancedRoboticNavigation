...
def estimate_pose(data, camera_matrix, dist_coeffs, tag_corners_world):
    ...
    """
    if len(data['id']) == 0:
        return None, None  # No tags detected
...
def process_data(file_path, camera_matrix, dist_coeffs, tag_corners_world):
    """Processes the dataset and estimates pose for each time step."""
    ...
    true_positions = vicon[:, :3]
    true_orientations = vicon[:, 3:6]
    ...
    return np.array(estimated_positions), np.array(estimated_orientations), true_positions, true_orientations
...
file_path = r"C:\Users\camil\Documents\WPI\RBE595-ARN\Assignment 2\data"

estimated_positions, estimated_orientations, true_positions, true_orientations = process_data(
    file_path, camera_matrix, dist_coeffs, tag_corners_world
)
